...
var (
	...
	cacheGozeroMallProductIdPrefix = "cache:gozeroMall:product:id:"
)
...
func (m *defaultProductModel) Delete(ctx context.Context, id uint64) error {
	gozeroMallProductIdKey := fmt.Sprintf("%s%v", cacheGozeroMallProductIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, gozeroMallProductIdKey)
	return err
}
...
func (m *defaultProductModel) FindOne(ctx context.Context, id uint64) (*Product, error) {
	gozeroMallProductIdKey := fmt.Sprintf("%s%v", cacheGozeroMallProductIdPrefix, id)
	var resp Product
	err := m.QueryRowCtx(ctx, &resp, gozeroMallProductIdKey, func(ctx context.Context, conn sqlx.SqlConn, v any) error {
...
func (m *defaultProductModel) Insert(ctx context.Context, data *Product) (sql.Result, error) {
	gozeroMallProductIdKey := fmt.Sprintf("%s%v", cacheGozeroMallProductIdPrefix, data.Id)
...
func (m *defaultProductModel) formatPrimary(primary any) string {
	return fmt.Sprintf("%s%v", cacheGozeroMallProductIdPrefix, primary)
}
